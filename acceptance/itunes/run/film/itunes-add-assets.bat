...
@echo on
%PROG% itunes -c ..\..\..\config.xml -m convert --imp %1 --cpl %2 -w %3 --vendor-id %4 --poster %5 --trailer %6 --chapters %7 -l debug -p film -d
@echo off
exit /b 0
...
@echo 6th argument - a path to trailer.
@echo 7th argument - a path to chapters.xml.
@echo 8th argument - a path to imf-conversion-utility (if not set a default one will be used assuming that we're in the source root folder).
@echo off
exit /b 1